...
w_pre = 0.1*randn(size(w2,2)+1,numoutput);
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    w_pre = reshape(X,l4+1,l5);

  else
    VV = [w1(:)' w2(:)' w_pre(:)']';
    Dim = [l1; l2; l4; l5];
    [X, fX] = minimize(VV,'CG_CLASSIFY2L',max_iter,Dim,data,targets);
...
%    xxx = xxx+(l3+1)*l4;
    w_pre = reshape(X(xxx+1:xxx+(l4+1)*l5),l4+1,l5);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 save mnistclassify_weights w1 w2 w_pre
