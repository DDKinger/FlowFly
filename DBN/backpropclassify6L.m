...
w_pre = 0.1*randn(size(w6,2)+1,numoutput);
...
N=numcases;
 for batch = 1:numbatches
  ...
  targetout = 1./(1 + exp(-w6probs*w_pre));
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    w4probs = 1./(1 + exp(-w3probs*w4)); w4probs = [w4probs  ones(N,1)];
    w5probs = 1./(1 + exp(-w4probs*w5)); w5probs = [w5probs  ones(N,1)];
    w6probs = 1./(1 + exp(-w5probs*w6)); %w5probs = [w3probs  ones(N,1)];

    VV = [w_pre(:)']';
    Dim = [l7; l8];
    [X, fX] = minimize(VV,'CG_CLASSIFY_INIT',max_iter,Dim,w6probs,targets);
    w_pre = reshape(X,l7+1,l8);

  else
    VV = [w1(:)' w2(:)' w3(:)' w4(:)' w5(:)' w6(:)' w_pre(:)']';
    Dim = [l1; l2; l3; l4; l5; l6; l7; l8];
    [X, fX] = minimize(VV,'CG_CLASSIFY6L',max_iter,Dim,data,targets);
    ...
    xxx = xxx+(l6+1)*l7;
    w_pre = reshape(X(xxx+1:xxx+(l7+1)*l8),l7+1,l8);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 save mnistclassify_weights w1 w2 w3 w4 w5 w6 w_pre
