...
w_pre = 0.1*randn(size(w4,2)+1,numoutput);
...
N=numcases;
 for batch = 1:numbatches
  ...
  targetout = 1./(1 + exp(-w4probs*w_pre));
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    w_pre = reshape(X,l5+1,l6);

  else
    VV = [w1(:)' w2(:)' w3(:)' w4(:)' w_pre(:)']';
    Dim = [l1; l2; l3; l4; l5; l6];
    [X, fX] = minimize(VV,'CG_CLASSIFY4L',max_iter,Dim,data,targets);
    ...
    xxx = xxx+(l4+1)*l5;
    w_pre = reshape(X(xxx+1:xxx+(l5+1)*l6),l5+1,l6);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 save mnistclassify_weights w1 w2 w3 w4 w_pre
